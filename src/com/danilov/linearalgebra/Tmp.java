...
public class Tmp {
	...
	public Tmp(ArrayList<String> matrix, int rows, int cols){
		this.matrix = new Fraction[rows + 1][cols + 1];
	...
	public Fraction[] getPolynomial(int n){
		Fraction[] coeffs = new Fraction[n + 2];
		int i,j,k,l;
		Fraction s;
		Fraction trace;
		Fraction[][] tmpB = new Fraction[n + 1][n + 1];
		createEmptyMatrix(tmpB, n + 1);
		Fraction[][] tmpC = new Fraction[n + 1][n + 1];
		createEmptyMatrix(tmpC, n + 1);
		if ((n % 2)!=0){ 
			coeffs[1]= new Fraction(-1);
		}else{ 
			coeffs[1]= new Fraction(1);
		}
		for (l = 1; l <= n; l++){
		    ...
		    coeffs[l+1]=trace.multiply(coeffs[1]).multiply(new Fraction(-1, 1));
		    if (l < n){ 
		      for (i=1; i<=n; i++){
		        for (j=1; j<=n; j++){
		          if (j==i) { 
